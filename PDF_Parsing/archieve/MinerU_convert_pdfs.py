...
class PDFConverter:
    ...
    def batch_process_pdfs(self,input_dir=None,output_dir=None,n_workers=8,method="atuo",lang=""):
        if input_dir is None:
            input_dir = self.input_dir
        if output_dir is None:
            output_dir = self.output_dir
        ...
        datasets = batch_build_dataset(pdf_files, n_workers, lang)
...
if __name__ == "__main__":
    ...
    if args.n_workers == 1:
        logging.info("Starting sigle_process_pdfs with single worker...")
        converter.process_all_pdfs()
    else:
        logging.info(f"Starting batch_process_pdfs with {args.n_workers} workers...")
        converter.batch_process_pdfs(n_workers=args.n_workers, method="auto", lang="en")
